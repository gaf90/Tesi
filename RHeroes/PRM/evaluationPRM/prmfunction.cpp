...
PRMFunction::PRMFunction(PRMAlgorithm* prm, PathPlannerPRM* planner, uint robotId): prmAlgorithm(prm), planner(planner), EvaluationFunction(robotId),
...
EvaluationRecords* PRMFunction::evaluateFrontiers(const QList<Frontier*> &f, const Map &map, int batteryTime, QHash<uint,double> &powerSignalData){
    ...
    foreach(Frontier* f, frontiers){
        ldbg<< "PRM Function: frontier: "<<f->centroid();
...
double PRMFunction::evaluateFrontier(const Frontier* frontier, const Map &map, int batteryTime, QHash<uint,double> &powerSignalData){
...
void PRMFunction::onEnableUserCriteria(bool activate, const Data::HighLevelCommand *command){
    //controllo del robot da parte dell'utente: manuale!
}

CriterionPRM* PRMFunction::createCriterion(QString name, double weight){
    ...
    if(name == QString(BATTERY_PRM)){
        ...
        toRet = new InformationGainCriterionPRM(weight);
